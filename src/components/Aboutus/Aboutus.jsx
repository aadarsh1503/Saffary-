...
const Aboutus = () => {
  ...
  return (
    <section id='about' className={`flex flex-col lg:flex-row ${i18n.language === 'ar' ? 'text-right max-w-7xl  ' : ''} items-center mt-2 max-w-7xl mx-auto justify-between py-10 lg:py-20 px-6 `}>
      <div className={`w-full lg:w-1/2 mb-6 ${i18n.language === 'ar' ? 'text-right lg:w-1/2 w-2/2 lg:mr-0    ' : ''} lg:mb-0`}>
      ...
      <div className="w-full lg:w-1/2">
        <img 
          src={g1} 
          alt={t('aboutSaffary')} 
          className={`lg:w-full w-[500px] h-[200px] ${i18n.language === 'ar' ? 'text-right lg:mr-0   ' : ''} lg:h-auto rounded-lg `}
        />
      </div>
    </section>
  );
